...
namespace SettlersOfIdlestan.Controller.Island
{
    ...
    public class MaritimeBeaconController
    {
        ...
        private readonly Dictionary<int, (int OccupiedCount, int RoadCount, int TileCount, bool AllowDeepWater, List<Vertex> Vertices)> _buildableVerticesCache = new();
        ...
        /// non profonde (TerrainType.Water strictement — ni terre, ni eau profonde cosmétique), ou
        /// d'un mélange d'Eau et d'Eau profonde si la civilisation possède
        /// MARITIME_BEACON_DEEP_WATER_PLACEMENT (Grotte aux Perles, Sirènes) ; non déjà occupés par
        /// une ville ou une balise (de n'importe quelle civilisation), et à distance d'au plus
        /// <see cref="MaxDistanceFromRoad"/> edge(s) d'une route de la civilisation (sinon les
        /// balises pourraient être posées n'importe où en pleine mer, sans lien avec le réseau
        /// routier existant). Résultat mis en cache par civilisation ; invalidé si le nombre
        /// d'emplacements occupés, de routes de la civilisation, de tuiles de la carte ou l'accès à
        /// l'Eau profonde change (ces deux avant-derniers évoluent toujours ensemble : construire une
        /// route étend la carte adjacente, voir AutoExtendController.TryExtendMapAfterRoad).
        /// </summary>
        public List<Vertex> GetBuildableVertices(int civilizationIndex)
        {
            ...
            bool allowDeepWater = civ.ModifierAggregator.HasModifier(ECategory.MARITIME_BEACON_DEEP_WATER_PLACEMENT);
            ...
            if (_buildableVerticesCache.TryGetValue(civilizationIndex, out var cached)
                ...
                && cached.AllowDeepWater == allowDeepWater)
            ...
            foreach (var layer in _state.Layers.Values)
            {
                ...
                foreach (var vertex in candidateVertices)
                {
                    ...
                    if (!IsFullyWater(map, vertex, allowDeepWater)) continue;
            ...
            _buildableVerticesCache[civilizationIndex] = (occupiedCount, roadCount, tileCount, allowDeepWater, result);
            return result;
        }
        ...
        /// <summary>
        /// Détruit toutes les Balises Maritimes dont le vertex n'est plus entouré de 3 hexs d'eau
        /// (non profonde stricte, ou Eau profonde comprise pour une balise dont la civilisation
        /// possède MARITIME_BEACON_DEEP_WATER_PLACEMENT) — même condition que
        /// <see cref="GetBuildableVertices"/> — et retourne celles qui sont tombées. À appeler après
        /// toute transformation de terrain, aujourd'hui uniquement Marche de Dieu
        /// (AscensionController.ApplyWalkOfGod), qui peut aussi bien assécher un hex d'eau sous une
        /// balise existante (favoured terrain = Eau, Sirènes) que la laisser intacte. Une balise
        /// détruite ainsi n'est pas assez pour retirer une Flotte de Guerre posée dessus (entité
        /// indépendante) — voir <see cref="WarFleetController.DestroyFleetsInvalidatedByTerrain"/>.
        /// </summary>
        public IReadOnlyList<MaritimeBeacon> DestroyBeaconsInvalidatedByTerrain()
        {
            ...
            foreach (var beacon in _state.GetAllMaritimeBeacons().ToList())
            {
                ...
                var civ = _state.GetCivilization(beacon.CivilizationIndex);
                bool allowDeepWater = civ != null && civ.ModifierAggregator.HasModifier(ECategory.MARITIME_BEACON_DEEP_WATER_PLACEMENT);
                if (map != null && IsFullyWater(map, beacon.Position, allowDeepWater)) continue;
        ...
        /// <summary>Vrai si les 3 hexs du vertex existent tous et sont de l'Eau (non profonde stricte, ou Eau profonde comprise si <paramref name="allowDeepWater"/>).</summary>
        private static bool IsFullyWater(IslandMap map, Vertex vertex, bool allowDeepWater)
            => vertex.GetHexes().All(h => map.Tiles.TryGetValue(h, out var tile)
                && (allowDeepWater ? tile.TerrainType.IsWater() : tile.TerrainType == TerrainType.Water));
    }
}
