...
namespace SettlersOfIdlestan.Model.GameplayModifier
{
    public class Modifier
    {
        ...
        public enum ECategory
        {
            ...
            /// <summary>Fraction de réduction de la <b>croissance</b> du coût de Purification des Os Divins : rabote le terme en N de l'exposant (voir DivineBones.GetCostMultiplier), pas le coût de base. Base = 0.0; 0.1 = exposant 1 + N/2 × 0,9.</summary>
            ...
            /// <summary>Bonus additif au plafond de Dominion que la production d'un Temple peut atteindre, par niveau de Temple (base : 2/niveau, voir CorruptionController.GetTempleDominionCap). Base = 0. Aucune source ne l'accorde actuellement (le Dogme de l'Emprise, qui le portait, allège désormais DOMINION_LAYER_PENALTY_REDUCTION).</summary>
            TEMPLE_DOMINION_CAP,
            ...
            /// <summary>Points de pourcentage, <b>par niveau effectif de Temple</b>, de chance qu'un Temple produise un second point de Dominion dans le même intervalle (voir CorruptionController.RollDoubleProduction). Base = 0 ; Humains 2 (soit 6% à niveau effectif 3, 10% à 5). Portait auparavant un bonus au débordement passif, supprimé avec lui.</summary>
            DOMINION_SPREAD_CHANCE,
            /// <summary>Réduction <b>multiplicative</b> (0-1), appliquée une fois <b>par Os Divin purifié sur l'île courante</b> (RunRecord.DivineBonesPurified), du risque qu'un Dominion situé sur un hex d'une ville possédant un Temple perde un niveau quand la production d'une source de Corruption tombe sur lui : risque = (1 − valeur)^Os, chance de résister = 1 − 0,9^Os avec 0,1 (voir CorruptionController.IsDominionSpared). Base = 0.0 ; Terre Consacrée 0,1 (soit −10%/Os).</summary>
            TEMPLE_DOMINION_PROTECTION_CHANCE,
            /// <summary>Probabilité (0-1) <b>par Os Divin purifié sur l'île courante</b> (RunRecord.DivineBonesPurified) qu'une réduction de Corruption infligée par une production de Dominion retire un niveau de plus. Au-delà de 100%, la tranche pleine garantit le second niveau et le reste est tiré pour un troisième (13 Os : second garanti, troisième à 30%) ; jamais plus de 3 niveaux d'un coup (voir CorruptionController.RollExtraCleanseLevels). Base = 0.0 ; Évangélisation 0,1 (soit 10%/Os).</summary>
            ...
            /// <summary>Bonus additif de vitesse de construction automatique des avant-postes par la Guilde des bâtisseurs, par ville possédée. Base = 0.0 ; agrégé puis multiplié par le nombre de villes de la civilisation (voir CityBuilderController.PerformBuildersGuildOutpostConstruction). 0.02 = +2% par ville. Volontairement distinct de GUILD_AUTOMATION_SPEED_PER_CITY (bâtiments) et bien plus faible : chaque avant-poste fonde une ville, qui accélère à son tour la cadence — le taux par ville doit rester petit pour que la boucle ne s'emballe pas.</summary>
            GUILD_AUTOMATION_OUTPOST_SPEED_PER_CITY,
            /// <summary>Vol (Garudas) : autorise la fondation de villes sans connexion routière, jusqu'à Value arêtes d'une ville de la civilisation posée sur la même couche ; le vertex doit toucher au moins un hex terrestre (le survol de l'eau est permis, pas l'atterrissage en pleine mer). Les autres règles (occupation, distances, terrain) s'appliquent normalement. Fonctionne en surface et dans l'Abysse, jamais dans l'Inframonde, et ne franchit pas les couches (voir CityBuilderController.AddFlightCandidateVertices). Base = 0 (pas de vol).</summary>
            ...
            /// <summary>Génération passive de Cristal par cycle (voir HarvestController.PassiveCrystalGenerationIntervalTicks), par Laboratoire construit (niveau ≥ 1) dans la civilisation. Base = 0.0 ; agrégé puis multiplié par le nombre de Laboratoires, avec report du reste fractionnaire d'un cycle à l'autre (voir HarvestController.PerformLaboratoryCrystalGeneration). 0.05 = +0.05 cristal par cycle et par laboratoire.</summary>
            ...
            UNLOCK_OBSERVATORY,
            /// <summary>Flags que la construction de la Nécropole (Monument, uniquement sur un hex portant des Os Divins non purifiés, qu'elle consomme) est déverrouillée pour la civilisation. Chaque niveau de la Nécropole augmente de 10% les points divins gagnés à l'Ascension (voir Necropolis et AscensionController.GetGodPointsGain). SubCategory unused.</summary>
            UNLOCK_NECROPOLIS,
