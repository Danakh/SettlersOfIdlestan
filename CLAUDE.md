...
- **Quantités lisibles par le joueur brouillées en mémoire** : toute quantité que le joueur lit à l'écran et qu'un éditeur de mémoire voudrait modifier se déclare en `ObfInt`/`ObfLong` (`Model/Obfuscation/`), jamais en `int`/`long` nu — stock de ressources, points de recherche et avancement de la recherche active, points de prestige, points divins, essences divines, soldats et défense courante d'un `IMilitaryVertex`. Ces types décomposent la valeur en deux moitiés retirées au hasard **à chaque écriture**, dont la somme n'est la valeur qu'une fois démultipliée par une clé tirée au démarrage du processus : ni la valeur affichée ni son incrément n'apparaissent en mémoire, et les deux mots changent de signe d'une écriture à l'autre. Les conversions implicites dans les deux sens rendent le changement transparent pour les appelants — une méthode publique qui prend ou rend un `int` (`Civilization.GetResourceQuantity`) n'a pas à changer de signature. Restent volontairement en `int` nu : les maximums et les coûts, recalculés à chaque lecture depuis les bâtiments et les modificateurs plutôt que stockés, et les ticks. La sérialisation n'écrit que la valeur réelle (la sauvegarde est chiffrée par ailleurs), donc les anciennes sauvegardes se relisent sans remap. **Une limite du typage C#** : `Assert.Equal(5, civ.GodPoints)` ne compile pas — l'inférence générique de xunit ne peut pas choisir entre `int` et `ObfInt`, les deux se convertissant l'un dans l'autre. Écrire `Assert.Equal<int>(5, civ.GodPoints)`. Rien d'autre dans le code n'est concerné.
