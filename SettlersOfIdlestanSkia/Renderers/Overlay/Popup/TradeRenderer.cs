...
public sealed class TradeRenderer : PopupRendererBase
{
    ...
    // Trade-specific polices (non-scalées — initialisées dans Initialize)
    ...
    public void Render(SKCanvas canvas)
    {
        ...
        SkiaTextUtils.DrawText(canvas, _localization.Get("trade_title"), popup.MidX, popup.Top + 28, SKTextAlign.Center, TitleFont!, TextPaint);
    ...
    private void DrawSellSide(SKCanvas canvas, float x, float y)
    {
        ...
        SkiaTextUtils.DrawText(canvas, _localization.Get("trade_give"), x + ColWidth / 2, y + 16, SKTextAlign.Center, BtnFont!, TextPaint);
        ...
        foreach (var resource in ResourceUtils.BasicResources.Where(r => tc.CanTradeResource(civ, r)))
        {
            ...
            SkiaTextUtils.DrawText(canvas, sellName, iconX + IconSize + 5, row.MidY + 5, BodyFont!, canSell ? TextPaint : SubtlePaint);

            SkiaTextUtils.DrawText(canvas, $"{available}/{maxQty}", row.MidX, row.MidY + 5, SKTextAlign.Center, _smallFont, SubtlePaint);
    ...
    private void DrawBuySide(SKCanvas canvas, float x, float y)
    {
        ...
        SkiaTextUtils.DrawText(canvas, _localization.Get("trade_advanced_title"), x + ColWidth / 2, y + 16, SKTextAlign.Center, BtnFont!, TextPaint);
        ...
        foreach (var resource in buyable)
        {
            ...
            SkiaTextUtils.DrawText(canvas, buyName, row.Left + 8 + IconSize + 5, row.MidY + 5, BodyFont!, canBuy ? TextPaint : SubtlePaint);

            SkiaTextUtils.DrawText(canvas, $"{qty}/{maxQty}", row.MidX, row.MidY + 5, SKTextAlign.Center, _smallFont, SubtlePaint);
    ...
    private SKRect DrawActionButton(SKCanvas canvas, SKRect row, string text, bool active)
    {
        ...
        SkiaTextUtils.DrawText(canvas, text, btn.MidX, btn.MidY + 5, SKTextAlign.Center, BodyFont!, active ? TextPaint : SubtlePaint);
        return btn;
    }
    ...
    private void DrawBottomBar(SKCanvas canvas, SKRect popup)
    {
        ...
        if (civ != null)
        {
            ...
            SkiaTextUtils.DrawText(canvas, qtyText, iconX + goldIconSize + 5, capsule.MidY + 4, _smallFont, TextPaint);
        }
    ...
    private void DrawMultButton(SKCanvas canvas, SKRect rect, string label, bool isActive, bool isTemporary = false)
    {
        ...
        SkiaTextUtils.DrawText(canvas, label, rect.MidX, rect.MidY + 5, SKTextAlign.Center, BodyFont!,
    ...
    private void DrawSeaportL3Checkbox(SKCanvas canvas, SKRect row, Resource resource, IReadOnlyList<Resource> enhanced, Civilization civ)
    {
        ...
        if (isOn)           SkiaTextUtils.DrawText(canvas, "4",  cb.MidX, cb.MidY + 5, SKTextAlign.Center, BodyFont!, TextPaint);
    ...
    private void DrawSeaportL4Checkbox(SKCanvas canvas, SKRect row, Resource resource, IReadOnlyList<Resource> autoTrade, Civilization civ)
    {
        ...
        canvas.DrawRoundRect(cb, 3, 3, _checkboxFillPaint);
        canvas.DrawRoundRect(cb, 3, 3, _checkboxBorderPaint);
        if (isOn)           SkiaTextUtils.DrawText(canvas, "A",  cb.MidX, cb.MidY + 5, SKTextAlign.Center, BodyFont!, TextPaint);
    ...
    private void DrawConfirmationPopup(SKCanvas canvas, SKRect parent, Resource resource, bool isAutoTrade)
    {
        ...
        SkiaTextUtils.DrawText(canvas, string.Format(_localization.Get(msgKey), resourceName), _confirmPopupRect.MidX, py + 42, SKTextAlign.Center, BodyFont!, TextPaint);
        SkiaTextUtils.DrawText(canvas, _localization.Get(permanentKey), _confirmPopupRect.MidX, py + 64, SKTextAlign.Center, _smallFont, SubtlePaint);
        ...
        SkiaTextUtils.DrawText(canvas, _localization.Get("trade_seaport_confirm_yes"), _confirmYesRect.MidX, _confirmYesRect.MidY + 5, SKTextAlign.Center, BtnFont!, TextPaint);
        SkiaTextUtils.DrawText(canvas, _localization.Get("trade_seaport_confirm_no"),  _confirmNoRect.MidX,  _confirmNoRect.MidY  + 5, SKTextAlign.Center, BtnFont!, TextPaint);
    }
    ...
    {
        float w = Math.Min(PopupWidth,  CanvasSize.Width  - 30);
        float h = Math.Min(PopupHeight, CanvasSize.Height - 30);
        return new SKRect((CanvasSize.Width - w) / 2, (CanvasSize.Height - h) / 2,
                          (CanvasSize.Width + w) / 2, (CanvasSize.Height + h) / 2);
    }
